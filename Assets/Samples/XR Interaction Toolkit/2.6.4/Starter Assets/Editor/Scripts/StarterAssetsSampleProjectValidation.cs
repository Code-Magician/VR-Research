            ...
            if (s_BuildValidationRules.Count == 0)
            {
                ...
                s_BuildValidationRules.Add(
                    new BuildValidationRule
                    {
                        ...
                        Category = k_Category,
                        CheckPredicate = () => (PlayerInput.all.FirstOrDefault()?.actions == null) || PackageVersionUtility.GetPackageVersion(k_InputSystemPackageName) >= s_RecommendedPackageVersion,
                        FixIt = () =>
                        {
                        ...
                        FixItAutomatic = true,
                        Error = (PlayerInput.all.FirstOrDefault()?.actions != null
) && (PlayerInput.all.FirstOrDefault()?.actions?.name
 == k_InputActionAssetName || AssetDatabase.AssetPathToGUID(AssetDatabase.GetAssetPath(PlayerInput.all.FirstOrDefault()?.actions
